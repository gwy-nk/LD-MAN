...
class AttentionWordRNN(nn.Module):
    ...
    def forward(self, words, sen_len):
        ...
        # x = self.norm1(output_word + self.dropout1(
        #     self.mhatt(output_word, output_word, output_word, mask)
        # ))
        #
        ...
        """
        # word_squish = F.tanh(self.h_h_linear(output_word))
        # word_attn = self.h_1_linear(word_squish)
        # word_attn_norm = F.softmax(word_attn.squeeze(), -1)
        #
        # word_attn_vectors = torch.sum(
        #     torch.mul(
        #         output_word,
        #         word_attn_norm.unsqueeze(-1).expand(word_attn_norm.size(0), word_attn_norm.size(1),
        #                                             output_word.size(-1))
        #     ),
        #     1
        # )
...
class AttentionSentRNN(nn.Module):
    ...
    def forward(self, word_attention_vectors,
        ...
        # TODO: Add attention mask CHECK!!
        # x = self.norm1(output_sent + self.dropout1(
        #     self.mhatt1(output_sent, output_sent, output_sent, lang_mask)
        # ))

        # x = self.norm2(x + self.dropout2(
        #     self.mhatt2(x_v, x_v, x, img_mask.transpose(-1, -2))
        # ))
        #
        # x = self.norm2(x + self.dropout2(
        #     self.ffn(x)
